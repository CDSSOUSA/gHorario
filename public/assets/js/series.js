...
var success = 'success';
const URL_SERIES = 'http://localhost/gerenciador-horario/public';

listSeries();

async function listSeries() {
    await axios.get(`${URL_SERIES}/series/list`)
        .then(response => {
            const data = response.data;
            console.log(data);
            document.querySelector("#tb_series > tbody").innerHTML = `${loadDataTable(data)}`;
            //loadDataTable(data)
        }
        )
...
function loadDataTable(data) {
    ...
    data.forEach((element, indice) => {
        ...
        row +=
            `<tr>
                <td>${indice + 1}</td>
                <td>${element.description}º ${element.classification} - ${convertShiftA(element.shift)} </td>
